...
class SampleGenerator(object):
    ...
    def __init__(self, data_distr: SimpleDataDistr | StarkSimplexDistribution, num_batches: int, batch_size: int = 1, **unused_kwargs):
...
class SimpleDataDistr(object):
    ...
    def __init__(self, dataset_params: DictConfig, joint_pmf):
    ...
    def sample(self, batch_size: int | tuple[int]) -> torch.Tensor:
        ...
        y = sample % self.c
        return torch.stack([x, y], dim=-1).reshape(*batch_size, 2)

    def sample_assignment(self, batch_size: int | tuple[int]) -> torch.Tensor:
        """
    ...
    def sample_smoothed_assignment(self, batch_size: int | tuple[int]) -> torch.Tensor:
        """
    ...
    def mse_from_hist(self, p: torch.Tensor) -> torch.Tensor:
        """
...
class CoupledBinaryDistribution(SimpleDataDistr):
    ...
    def tensor_format(self) -> tuple[int, int, int]:
        """
...
class StarkSimplexDistribution(object):
    ...
    def sample_assignment(self, batch_size: int) -> torch.Tensor:
        """
        Same as self.sample, but returns an integer assignment matrix 
    ...
    def sample_smoothed_assignment(self, batch_size: int) -> torch.Tensor:
        """
        Draw sample w on assignment manifold coner and return 
        smoothing*barycenter + (1-smoothing)*w
        output has shape (batch_size, c, n), n=2
