...
from architecture import UNet, DenseNet, DirichletCNN
...
class FlowMatchingModule(L.LightningModule):
	...
	def log_time_density(self):
		...
		fig.tight_layout()
		self.logger.experiment.add_figure("t_sampling_density", fig, self.global_step)

	@torch.no_grad()
	def generate_samples(self, num_samples=1, return_meta=False, return_prob=False):
		device = self.device
	...
	def training_step(self, batch, batch_idx):
		loss = matching_step(batch, self.net, self.training_params, dim=self.channel_dim)
		self.log("loss", loss)
		return loss
...
class ImageFlow(FlowMatchingModule):
	...
	def __init__(self, hparams):
		...
		self.net = UNet(image_size=32, in_channels=self.c, out_channels=self.c, **self.model_params)
	...
	def validation_step(self, batch, batch_idx):
		...
		self.logger.experiment.add_image('samples', sample_img_grid, self.global_step)
...
class ToyDataFlow(FlowMatchingModule):
	...
	def on_validation_epoch_end(self):
		...
		if self.n == 2:
			...
			ax.axis("off")
			fig.tight_layout()
			self.logger.experiment.add_figure("pmf_hist", fig, self.global_step)
