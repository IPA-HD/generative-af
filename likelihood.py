...
def tangent_to_coord(Q, v, dim=1):
	"""
...
def af_as_cnf(net, Q, t_end=1.0, div_method="hutchinson_gauss", hutchinson_samples=100, atol=1e-2, rtol=1e-2, dim=1, reverse_time=True):
	c = Q.shape[0]
...
def radius_decision_disk(v_alpha, dim=1):
	"""
...
def sigma_from_disk_mass(dim, r, mass):
	b = chi2.ppf(mass, dim)
	return r / math.sqrt(b)

def sample_log_likelihood(
	net, w_alpha, num_samples, proposal_mass=0.8,
	div_method="exact_ek", hutchinson_samples=100,
	dim=1, reduce_samples=True, rounding=True,
	rtol=1e-2, atol=1e-2, t_end=1.0):
